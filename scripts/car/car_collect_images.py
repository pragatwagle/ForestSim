...
while capture:
    response = client.simGetImages([airsim.ImageRequest(0, airsim.ImageType.Scene, False, False), 
                                    #currently not saving depth array, not working on windows due to pixels as float true
    ...
    # this will collect an image every 5 seconds
    if(time_span> 5):
        
        # the image name to save
        img_name = environment + '_' + today_date + "_" + str(picture) + "_" + "fpv";
        
        # write the images to a specified output directory, please replace the output path with your output path
        airsim.write_png(os.path.normpath(outputPath + environment + '/rgb/' + img_name + '_rgb' + '.png'), img_rgb) 
        airsim.write_png(os.path.normpath(outputPath + environment + '/depth/' + img_name + '_depth' + '.png'), img_depth) 
        airsim.write_png(os.path.normpath(outputPath + environment + '/segmentation/' + img_name + '_segmentation' + '.png'), img_seg) 
        ...
        with open(output_camerapose_folder + environment + '_' + today_date + '.csv', 'a+') as d:
            writer = csv.writer(d)
        ...
        # the count is then saved to specified folder 
        ...
        with open(output_class_count_folder + environment + '_' + today_date + '.csv', 'w+') as objectCountFile:
